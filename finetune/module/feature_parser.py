...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'lsh_frame_feature')
class LshFrameFeature(BaseFieldParser):
    def __init__(self):
        super().__init__()
        self.zero_frame = None
        self.num_segments = None
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'rsh_frame_feature')
class RshFrameFeature(BaseFieldParser):
    def __init__(self):
        super().__init__()
        self.zero_frame = None
        self.num_segments = None
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'lsh_tag_cls')
class Lsh_TagParser(BaseFieldParser):
    def __init__(self):
        super().__init__()
        self.selected_tags = set()
        self.mlb = MultiLabelBinarizer()
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'rsh_tag_cls')
class RshTagParser(BaseFieldParser):
    def __init__(self):
        super().__init__()
        self.selected_tags = set()
        self.mlb = MultiLabelBinarizer()
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'lsh_id')
class LshVidParser(BaseFieldParser):
    ...
    def parse(self, row, training=False):
        ...
        """
        record = {}
        vid = [bytes(row['lsh_id']).decode('utf-8')]
        record[self.alias] = vid
        return record
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'rsh_id')
class RshVidParser(BaseFieldParser):
    ...
    def parse(self, row, training=False):
        ...
        """
        record = {}
        vid = [bytes(row['rsh_id']).decode('utf-8')]
        record[self.alias] = vid
        return record
...
@plugin.register_plugin(plugin.PluginType.FIELD_PARSER, 'similarity')
class SimilarityParser(BaseFieldParser):

    def parse(self, row, training=False):
        """
        :param row: raw feature map {key1: raw_feature1, key2: raw_feature2, ...}
        :param training: training or not can behave different
        :return: similarity feature with {self.alias: feature}
        """
        record = {}
        similarity = row['similarity']
        record[self.alias] = similarity
        return record
