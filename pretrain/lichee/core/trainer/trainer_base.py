...
class TrainerBase:
        ...
        for step, batch in enumerate(self.train_dataloader):
            ...
            if self.cfg.RUNTIME.AUTOCAST:
                with torch.cuda.amp.autocast():
                    logits, loss_tag, loss_mlm = self.model(inputs)
            else:
                logits, loss_tag, loss_mlm = self.model(inputs)
            loss = loss_tag + loss_mlm
            loss = loss.mean()
            if self.cfg.RUNTIME.AUTOCAST:
            ...
            if (step + 1) % self.cfg.RUNTIME.REPORT_STEPS == 0:
                logging.info(
                    'step: {}/{}    loss_tag: {}    loss_mlm: {}   loss: {}'.format(step, len(self.train_dataloader), loss_tag, loss_mlm, loss))
    ...
    def eval_model(self, epoch):
        ...
        for step, batch in tqdm.tqdm(enumerate(self.eval_dataloader)):
            inputs = self.get_inputs_batch(batch)
            label_keys, labels = self.get_label_batch(batch)
            for i in range(len(label_keys)):
                inputs[label_keys[i]] = labels[i]
    ...
    def save_model(self, epoch):
        ...
        # decapsulation model
        torch_nn_convertor.TorchNNConvertor.save_model(self.model.module, None, tmp_epoch_model_file)
