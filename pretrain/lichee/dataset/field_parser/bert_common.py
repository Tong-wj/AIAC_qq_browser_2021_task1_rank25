...
def collate_bert_text(parser, batch):
    ...
    title_label = parser.alias + "_label"
    if title_label in batch[0]:
        batch_text_labels = [instance[title_label] for instance in batch]
    max_len = max([len(token_ids) for token_ids in batch_text_labels])
    for i in range(len(batch)):
        concate_bert_text_label(max_len, batch_text_labels, i)
    batch_text_labels = torch.stack(batch_text_labels)
    record[title_label] = batch_text_labels
...
def prepare_bert_text(parser, tokens, segment_ids, text_label):
    tokens = tokens[:parser.cfg["MAX_SEQ_LEN"]]
    text_label = text_label[:parser.cfg["MAX_SEQ_LEN"]]
    token_ids = parser.tokenizer.convert_tokens_to_ids(tokens)
    text_label_ids = parser.tokenizer.convert_tokens_to_ids(text_label)
    ...
    text_label_ids = torch.LongTensor(text_label_ids)
    return token_ids, text_label_ids
...
def concate_bert_text_label(max_len, bert_text_labels, i):
    pad_len = max_len - bert_text_labels[i].shape[0]
    if pad_len > 0:
        z = torch.zeros([pad_len], dtype=torch.long)
        bert_text_labels[i] = torch.cat([bert_text_labels[i], z], dim=0)
...
class BertTextFieldParserCommon(BaseFieldParser):
    ...
    def parse(self, row, training=False):
        ...
        record[self.alias], record[self.alias + "_label"] = self.prepare_text(text)
        record["org_" + self.alias] = text
        return record

    def prepare_text(self, text):
        split_text = self.tokenizer.tokenize(text)
        text_random, text_label = self.random_word(split_text)
        tokens = ["[CLS]"] + text_random + ["[SEP]"]
        tokens_labels = ["[PAD]"] + text_label + ["[PAD]"]
        segment_ids = [1] * len(tokens)

        return prepare_bert_text(self, tokens, segment_ids, tokens_labels)
        ...
        for i, token in enumerate(tokens):
            ...
            if prob < 0.15:
                ...
                # 10% randomly change token to random token
                    ...
                    token_original = tokens[i]
                    vocab_size = 21128
                    tokens[i] = random.randrange(0, vocab_size)
                    tokens[i] = self.tokenizer.convert_ids_to_tokens([tokens[i]])[0]
                    # print(tokens)
                ...
                output_label.append(token_original)
